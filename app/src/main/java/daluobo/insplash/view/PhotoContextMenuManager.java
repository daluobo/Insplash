 ...
 */
public class PhotoContextMenuManager extends RecyclerView.OnScrollListener implements View.OnAttachStateChangeListener {

    private static PhotoContextMenuManager instance;

    private PhotoContextMenu contextMenuView;

    private boolean isContextMenuDismissing;
    private boolean isContextMenuShowing;

    public static PhotoContextMenuManager getInstance() {
        if (instance == null) {
            instance = new PhotoContextMenuManager();
        }
        return instance;
    }

    private PhotoContextMenuManager() {

    }

    public void toggleContextMenuFromView(View openingView, int feedItem, PhotoContextMenu.OnPhotoContextMenuItemClickListener listener) {
    ...
    private void showContextMenuFromView(final View openingView, int feedItem, PhotoContextMenu.OnPhotoContextMenuItemClickListener listener) {
        if (!isContextMenuShowing) {
            isContextMenuShowing = true;
            contextMenuView = new PhotoContextMenu(openingView.getContext());
            contextMenuView.bindToItem(feedItem);
            contextMenuView.addOnAttachStateChangeListener(this);
            contextMenuView.setOnMenuItemClickListener(listener);
    ...
    private void setupContextMenuInitialPosition(View openingView) {
        ...
        int additionalBottomMargin = DimensionUtil.dpToPx(16);
